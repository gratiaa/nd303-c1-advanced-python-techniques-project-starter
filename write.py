...
def write_to_csv(results, filename):
    ...
    with open(filename, mode='w') as file:
        ...
        for result in results:
            neo = NearEarthObject(
                result.neo.designation, result.neo.name, result.neo.diameter, result.neo.hazardous).serialize()
...
def write_to_json(results, filename):
    ...
    with open(filename, mode='w') as file:
        list = []
        for result in results:
            neo = NearEarthObject(result.neo.designation, result.neo.name,
                                  result.neo.diameter, result.neo.hazardous).serialize()
            approach = CloseApproach(
                result.neo.designation,
                result.time, result.distance, result.velocity).serialize()
            approach['neo'] = neo
